...
public class MethodReferences {
    ...
    public static void exercise1() {
        ...
        List<Integer> evens = numbers.stream().filter(MethodReferences::isEven).toList(); // TODO: filter using
    ...
    public static void exercise3() {
        ...
        List<String> lowercased = words.stream().map(String::toLowerCase).toList(); // TODO: use String::toLowerCase
                                                                                    // with map
        System.out.println("Lowercased: " + lowercased);

        // B) String::length — applied to each element
        List<Integer> lengths = words.stream().map(String::length).toList(); // TODO: use String::length with map
        System.out.println("Lengths: " + lengths);

        // C) String::isEmpty — applied to each element
        List<String> nonEmpty = words.stream().filter(Predicate.not(String::isEmpty)).toList(); // TODO: use
                                                                                                // String::isEmpty with
                                                                                                // filter (negate it!)
    ...
    public static void exercise4() {
        ...
        List<String> strings = numbers.stream().map(String::valueOf).toList(); // TODO: use String::new with map
    ...
    public static void exercise5() {
        ...
        String result = names.stream().filter(MethodReferences::startsWithLetterJ).map(String::toUpperCase)
